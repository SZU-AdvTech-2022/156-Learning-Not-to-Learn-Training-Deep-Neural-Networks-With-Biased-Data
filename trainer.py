...
class Trainer(object):
    ...
    def _set_optimizer(self):
        self.optim = optim.SGD(filter(lambda p: p.requires_grad, self.net.parameters()), lr=self.option.lr,
                               momentum=self.option.momentum, weight_decay=self.option.weight_decay)
        self.optim_r = optim.SGD(self.pred_net_r.parameters(), lr=self.option.lr, momentum=self.option.momentum,
                                 weight_decay=self.option.weight_decay)
        self.optim_g = optim.SGD(self.pred_net_g.parameters(), lr=self.option.lr, momentum=self.option.momentum,
                                 weight_decay=self.option.weight_decay)
        self.optim_b = optim.SGD(self.pred_net_b.parameters(), lr=self.option.lr, momentum=self.option.momentum,
                                 weight_decay=self.option.weight_decay)
    ...
    def _train_step(self, data_loader, step):
        ...
        for i, (images, color_labels, labels) in enumerate(data_loader):
            ...
            # loss for self.net
            loss_pred = self.loss(pred_label, torch.squeeze(labels.long()))
    ...
    def _validate(self, data_loader):
        ...
        for i, (images, color_labels, labels) in enumerate(data_loader):
            ...
            loss = self.loss(pred_label, torch.squeeze(labels.long()))
